...
from src.app.api.routes import admin
...
def test_create_pipeline_run_returns_202_pending_immediately(monkeypatch) -> None:
    ...
    monkeypatch.setattr(admin, "create_pipeline_run", fake_create_pipeline_run)
    ...
    assert response.headers["location"] == "/api/admin/runs/run-1"
...
def test_get_pipeline_run_returns_durable_run(monkeypatch) -> None:
    ...
    response = TestClient(create_app()).get(
        "/api/admin/runs/run-1", headers=ADMIN_HEADERS
    )
...
def test_get_pipeline_run_returns_404_for_unknown_run_id(monkeypatch) -> None:
    monkeypatch.setattr(admin, "get_pipeline_status", lambda run_id: None)
    monkeypatch.setattr(auth, "get_settings", admin_settings)
    response = TestClient(create_app()).get(
        "/api/admin/runs/missing", headers=ADMIN_HEADERS
    )
...
def test_create_pipeline_run_rejects_invalid_input(monkeypatch) -> None:
    ...
    monkeypatch.setattr(admin, "create_pipeline_run", reject)
    monkeypatch.setattr(auth, "get_settings", admin_settings)
    response = TestClient(create_app()).post(
        "/api/admin/pipeline/run", headers=ADMIN_HEADERS, json={"input_data": {}}
    )
...
def test_pipeline_start_requires_configured_bearer_token(monkeypatch) -> None:
    ...
    monkeypatch.setattr(admin, "create_pipeline_run", lambda input_data: pending_record())
    client = TestClient(create_app())

    assert client.post(
        "/api/admin/pipeline/run", json={"input_data": {"gameweek": 32}}
    ).status_code == 401
    assert client.post(
        "/api/admin/pipeline/run",
